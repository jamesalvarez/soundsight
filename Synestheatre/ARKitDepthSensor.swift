...
class ARKitDepthSensor : NSObject, DepthSensor, ARSessionDelegate {
    ...
    func getDepthInMillimeters(_ outArray: UnsafeMutablePointer<Float>!) -> Bool {
        ...
        guard let sceneDepth = currentFrame.smoothedSceneDepth ?? currentFrame.sceneDepth else {
    ...
    func getColours(_ outArray: UnsafeMutablePointer<Colour>!) -> Bool {
        ...
        var pixels = [UInt32](repeating: 0, count: height * width)
        
        
        let colorSpace = CGColorSpaceCreateDeviceRGB();

        pixels.withUnsafeMutableBytes { pixelsPtr in

            let ctx = CGContext(data: pixelsPtr.baseAddress!, width: width, height: height, bitsPerComponent: bitsPerComponent, bytesPerRow: bytesPerRow, space: colorSpace, bitmapInfo: CGBitmapInfo.byteOrder32Big.rawValue);
