...
- (BOOL)canLoadInSynestheatre {
    if (
        ...
        self.type == MLBFileTypeMP4 ||
        self.type == MLBFileTypeAVI ||
        self.type == MLBFileTypeMOV ||
        self.type == MLBFileTypeMPG ||
        self.type == MLBFileTypeWMV
        ) return YES;
    ...
    if (isExists && isDir) {
        ...
        if (!error) {
            for (NSString *name in contents) {
                ...
                if ([fileInfo canLoadInSynestheatre]) {
