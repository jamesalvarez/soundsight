...
- (void)trigger {
    ...
    if (rootViewController.presentedViewController) {
        if (rootViewController.presentedViewController == self.homeDirectoryNavigationController) {
            ...
            [self.homeDirectoryNavigationController dismissViewControllerAnimated:YES completion:^{
                
                if ([[Sandboxer shared] hasSelectedURL]) {
                    [[NSUserDefaults standardUserDefaults] setURL:[[Sandboxer shared] selectedURL] forKey:@"selected_file"];
                    NSLog(@"Saved selected file");
